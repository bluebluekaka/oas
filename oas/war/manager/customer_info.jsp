...
<head><title>
    ...
    <script type="text/javascript"> 
        ...
        $(function () {	
			...
			var checkTelUrl = '<%=url%>?action=checkTelephone&customerID='+customerID+'&t='+Math.random(100000);
			var checkQqUrl = '<%=url%>?action=checkQq&customerID='+customerID+'&t='+Math.random(100000);
			var v = $("form").validate({
                debug: false,
                rules: {
					...
					"customer.telephone":{required:true,minlength:6,maxlength:20,remote:checkTelUrl},
					                                      <%-- "user.uid": {required:true,minlength:3,maxlength:10,remote:'<%=url%>?action=check&old='+'<s:property value="user.uid"/>'+'&t='+Math.random(100000)}, --%>       
					"customer.phone":{minlength:6,maxlength:20 },
					"customer.qq":{required:true,minlength:4,maxlength:20,remote:checkQqUrl},
					"customer.address":{minlength:6,maxlength:40 },
					"customer.residualValue":{minlength:1,maxlength:10 },
					"customer.company":{required:true},
					"customer.sex":{required:true}
...
<table cellpadding="0" cellspacing="0" class="l-table-edit" >
            ...
            <% if(utype==Config.USER_TYPE_TOP_MANERGER||"add".equals(actionType)){ %>
              <tr>
            ...
            <%} else{ if(flag.equals("true")&& (utype==Config.USER_TYPE_COMMON || utype== Config.USER_TYPE_COMMON_MANERGER)){%>
             <tr>
                <td align="right" class="l-table-edit-td">* 手机号码:</td>
                <td align="left" class="l-table-edit-td"><input name="customer.telephone" id="telephone" ltype="text" value="<s:property value='customer.telephone'/>" /></td>  <td align="left"></td>
            </tr>
 ...
 <br />
  ...
  <input type="submit" value="提交" id="Button1" class="l-button l-button-submit" /> 
  <%}if(flag.equals("true")&&utype==Config.USER_TYPE_COMMON_MANERGER){ %>
  <input type="submit" value="提交" id="Button1" class="l-button l-button-submit" /> 
  <%} } %>
